...
def sendEmail(receiver_email, name, msg):
    ...
    sender_email = "sagarahire0505@gmail.com"
    sender_password = "vqbwyjcfnwypuucm"
    ...
    with open("birthday.gif",'rb') as f:
        image_data = f.read()
        image_type = 'gif'
        image_name = f.name
...
if __name__ == "__main__":
    ...
    for index, item in dataframe.iterrows():
        ...
        df.loc[df['name'] == 'sagar', 'birthday'] = '06/01/2023'
        df.loc[df['name'] == 'sagar', 'year'] = '2002'

        # Write the modified data back to the file
        df.to_excel("birthday.xlsx", index=False)

        # stripping the birthdays in excel sheet as : DD-MM
        bday_string = item['birthday'].strip('"')
        bday = datetime.datetime.strptime(bday_string, "%d/%m/%Y").strftime("%d-%m")
        name = item['name']
